...
class UIConfirmFlowSmokeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.temp_dir = Path(tempfile.mkdtemp(prefix="ui_confirm_flow_"))
        cls.db_path = cls.temp_dir / "watchkeeper_confirm_flow.db"
    ...
    def test_assist_then_confirm_records_event(self) -> None:
        request_id = "req-ui-confirm-001"
        incident_id = "inc-ui-confirm-001"
        ...
        self.assertTrue(preview[0].get("decision", {}).get("requires_confirmation"))
        token = str(preview[0].get("confirm_token") or "")
        ...
        self.assertIn(
            confirmed.get("tool_name"),
            {preview[0]["tool_name"], "input.keypress"},
        )
        ...
        try:
            row = con.execute(
                ...
                SELECT COUNT(*)
                FROM event_log
                WHERE event_type='ASSIST_CONFIRM_ACCEPTED'
